...
########################################################################################
f = open('C:\\Projects\\11\\WEB\\PyWEBArr.txt', "a")


for dd in diffs_file:
#        dd = dd.split('\t')
#        the_file.write(dd.replace('/', os.sep).replace('\\', os.sep).encode('cp1251'))
#        the_file.write(dd)
    f.writelines(dd.lower().replace('/', os.sep).replace('\\', os.sep) + '\n')
...
for diff_line in diffs_file:
    ...
    if diff_line[0] == 'M' or diff_line[0] == 'A':
        dst_path = os.path.join(release_dir, diff_line[1].replace('/',os.sep))
        src_path = os.path.join(git_repo_dir, diff_line[1].replace('/',os.sep))
        #create directory if it not exists 
        os.makedirs(os.path.dirname(dst_path), exist_ok=True)
        ...
        dst_path = os.path.join(release_dir, diff_line[2].replace('/',os.sep).replace('\\',os.sep))
        src_path = os.path.join(git_repo_dir, diff_line[2].replace('/',os.sep).replace('\\',os.sep))
        #create directory if it not exists 
        os.makedirs(os.path.dirname(dst_path), exist_ok=True)
        copyfile(src_path, dst_path)
