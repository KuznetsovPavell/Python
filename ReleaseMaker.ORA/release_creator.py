...
'''
release_no = sys.argv[1] #'13.0.031'
tagfrom = sys.argv[2] #'TestRelease_PROJ_13.0.030'
...
repo.remotes.origin.pull(target_branch) #~ DisComment
...
if line != install_stop_line :
    with open(os.path.join(git_repo_dir, install_path), 'ab') as the_file:
        ...
        repo.git.add(os.path.join(git_repo_dir, install_path))

# ----WEB----

if os.path.exists(os.path.join(git_repo_dir, 'web',  'barsroot', 'version.abs')) :
...
if os.path.exists(os.path.join(git_repo_dir, 'web_rozdrib',  'barsroot', 'version.abs')) :
...
for diff_line in diffs_file:
    ...
    diff_line[1] = diff_line[1].lower().replace('/', os.sep).replace('\\', os.sep).replace(' ', '')
    if diff_line[0].upper() == 'M' or diff_line[0].upper() == 'A':
#        print(diff_line[0] + ';' + diff_line[1] + ' - ' + diff_line[2])
